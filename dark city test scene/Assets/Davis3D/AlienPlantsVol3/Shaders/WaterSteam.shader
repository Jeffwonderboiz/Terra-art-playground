...
Shader "Davis3D/AlienPlants_Vol3/Enviroment/WaterSteam"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _Flowmap_Speed;
			float _Flowmap_Intensity;
			float _Opacity;
			float _Camera_Fade_Distance;
			float _Camera_Fade_Hardness;
			float _Distance_Fade_Distance;
			float _Distance_Fade_Hardness;
					...
					)
			{
				...
				float screenDepth39 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth39 = saturate( abs( ( screenDepth39 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) ) );
				float2 texCoord21 = packedInput.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_2_0_g4 = ( _TimeParameters.x * _Flowmap_Speed );
				float2 texCoord28 = packedInput.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				...
				surfaceDescription.Alpha = saturate( ( ( ( distanceDepth39 * ( packedInput.ase_color.a * ( tex2D( _Texture, lerpResult20 ).g * _Opacity ) ) ) * ( 1.0 - pow( saturate( dotResult8_g17 ) , _Camera_Fade_Hardness ) ) ) * pow( saturate( dotResult8_g18 ) , _Distance_Fade_Hardness ) ) );
		...
		Pass
		{
					...
					)
			{
				...
				float screenDepth39 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth39 = saturate( abs( ( screenDepth39 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) ) );
				float2 texCoord21 = packedInput.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_2_0_g4 = ( _TimeParameters.x * _Flowmap_Speed );
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				o.ase_color = inputMesh.ase_color;
				o.ase_texcoord3.xy = inputMesh.uv0.xy;
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				float screenDepth39 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth39 = saturate( abs( ( screenDepth39 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) ) );
				float2 texCoord21 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_2_0_g4 = ( _TimeParameters.x * _Flowmap_Speed );
		...
		Pass
		{
					...
					)
			{
				...
				float screenDepth39 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth39 = saturate( abs( ( screenDepth39 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) ) );
				float2 texCoord21 = packedInput.ase_texcoord4.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_2_0_g4 = ( _TimeParameters.x * _Flowmap_Speed );
				float2 texCoord28 = packedInput.ase_texcoord4.xy * float2( 1,1 ) + float2( 0,0 );
				float2 panner26 = ( temp_output_2_0_g4 * float2( 0.1,0.1 ) + texCoord28);
				float2 lerpResult20 = lerp( texCoord21 , (tex2D( _FlowMap, ( _Flowmap_scale * panner26 ) )).rg , _Flowmap_Intensity);
...
ASEEND*/
//CHKSM=44040FA13D761A169550FF17AA21D5BB2CF6CD2E